...
class InputController extends Controller
{
    ...
    public function index(Request $request)
    {
        ...
        if($user['role_id'] == 1){
            $vend = DB::table('vendas')->where('user_name_id', $user['user_name'])->orderBy('created_at', 'ASC')->get();
        ...
        if($user['role_id'] == 3){
            $vend = DB::table('vendas')->where('supervisor_id', $user['id'])->orderBy('created_at', 'ASC')->get();
            return view('INPUT.input',compact('vend','supervisor_user','vendedor_user','num_contrato','nome','cpf','user_name_id','supervisor_id','END','START'));
        };
        if($user['role_id'] == 4){
            $vend = DB::table('vendas')->orderBy('created_at', 'ASC')->get();
            return view('INPUT.input',compact('vend','supervisor_user','vendedor_user','num_contrato','nome','cpf','user_name_id','supervisor_id','END','START'));
        };
