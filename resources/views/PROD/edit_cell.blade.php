...
<form class="row g-3 margin-top-12" method="post" action="{{ URL('/cell/update/'.$product->id) }}">
    @csrf
    @method('PUT')
    ...
    <div class="card mb-12 search-box">
          ...
          <div class="form-row">
            ...
            <input type="text" name="cell" value="{{$product->cell}}" class="form-control" >
            </div>
            ...
            <form action="{{ URL('/cell/destroy/'.$product->id) }}" method="POST">
            @csrf
            @method('DELETE')
