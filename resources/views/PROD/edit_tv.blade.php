...
<form class="row g-3 margin-top-12" method="post" action="{{ URL('/tv/update/'.$product->id) }}">
    @csrf
    @method('PUT')
   
    <div class="card mb-12 search-box">
          <div class="card-header text-header title-white">
            Nome da Grade de canais 
          </div>
          <div class="card-body">
          <div class="form-row">
            ...
            <input type="text" name="tv" value="{{$product->tv}}" class="form-control" >
            </div>
            ...
            <form action="{{ URL('/tv/destroy/'.$product->id) }}" method="POST">
            @csrf
            @method('DELETE')
